...
*Setup instructions for a Gitian build of Gust Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Gust
...
- [Building Gust Core](#building-gust-core)
...
Clone the git repositories for Gust Core and Gitian.
...
in the Gust Core repository under 'Fetch and build inputs' to install sources which require
...
To build Gust Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Gust Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gust/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/gustpay/gust
...
For example:
```bash
URL=https://github.com/crowning-/gust.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gust=${COMMIT} --url gust=${URL} ../gust/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gust=${COMMIT} --url gust=${URL} ../gust/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gust=${COMMIT} --url gust=${URL} ../gust/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gust git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gust/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/gust
SIGPATH=/some/root/path/gust-detached-sigs

./bin/gbuild --url gust=${BTCPATH},signature=${SIGPATH} ../gust/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gust-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gust-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gust-osx-build.assert
```
...
[gust/gitian.sigs](https://github.com/gustpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
