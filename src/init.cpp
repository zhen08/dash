...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "gust (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Gust specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N GUST anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/gustpay/gust>";
    const std::string URL_WEBSITE = "<https://gust.org>";
...
/** Sanity checks
 *  Ensure that Gust Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Gust Core version %s\n", FormatFullVersion());
}

/** Initialize Gust Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Gust Core process is using the data directory.
    ...
    // ********************************************************* Step 11c: update block tip in Gust modules
    ...
    // ********************************************************* Step 11d: start gust-ps-<smth> threads
