...
const char * const BITCOIN_CONF_FILENAME = "gust.conf";
const char * const BITCOIN_PID_FILENAME = "gustd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gust" is a composite category enabling all Gust-related debug output
            if(ptrCategory->count(string("gust"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GustCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GustCore
    // Mac: ~/Library/Application Support/GustCore
    // Unix: ~/.gustcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GustCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GustCore";
#else
    // Unix
    return pathRet / ".gustcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gust.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gust.conf
